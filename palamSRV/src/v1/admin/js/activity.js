import {
    ...
    obreDialogAfegeixLlistaBlanca, obre_confirmacio, obreDialogBloquejaWeb, obreDialogDebug
} from "./dialogs.js";
    ...
    for (let grup in grupAlumnesList) {
        for (let alumne in grupAlumnesList[grup].alumnes) {
            ...
            if (!document.getElementById(alumne + "-header")) {
                ...
                debugButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bug" viewBox="0 0 16 16">
                ...
                sendMessageButton.onclick = () => {
                    bootbox.prompt({
                            title: "Envia un missatge a " + alumne,
                            message: `Escriu el missatge que vols enviar a ${alumne}` +
                                `<br> <i> Només funciona amb la v2 de Palamblock i si l'usuari té el navegador actiu </i> <br><br>`,
                            callback: (message) => {
                        if (message)
                            socket.emit("sendMessageToAlumne", {alumne: alumne, message: message});
                    }});
                }
                alumneDivButtons.appendChild(document.createTextNode(' '))
                alumneDivButtons.appendChild(sendMessageButton);

                const screenButton = document.createElement("button");
                screenButton.setAttribute("id", alumne + "-screen-button");
                screenButton.setAttribute("class", "btn btn-dark");
                screenButton.setAttribute("type", "button");

                if(!alumneInfo || !alumneInfo.currentIp){
                ...
                screenButton.onclick = () => {
                    let url = "http://" + alumneInfo.currentIp + ":6080/vnc_lite.html?resize=scale&reconnect&name=" + alumne;
                    // Open new tab
                    let tab = window.open(url, '_blank');
                }

                alumneDivButtons.appendChild(document.createTextNode(' '))
                alumneDivButtons.appendChild(screenButton);
            }
