...
export function creaWebMenuJSON(
  alumne,
  browser,
  alreadyBlocked = false,
  searhOnHistorial = false
) {
...
function preparaSelectDurada(
  hSelectDurada,
  nowHM,
  opcioSeleccionada = "primera"
) {
  ...
  hSelectDurada.appendChild(
    new Option("Avui", "today", false, opcioSeleccionada === "today")
  );
  hSelectDurada.appendChild(
    new Option("Sempre", "always", false, opcioSeleccionada === "always")
  );
  hSelectDurada.appendChild(
    new Option(
      "Excepte al pati",
...
export function obreDialogBloquejaWeb(
  webPage,
  alumne,
  grup,
  tab,
  ...
  const pathnameSwitch = document.getElementById(
    "pbk_modalblockweb_pathname_switch"
  );
  const searchSwitch = document.getElementById(
    "pbk_modalblockweb_search_switch"
  );
  const titleSwitch = document.getElementById("pbk_modalblockweb_title_switch");
  const titleOptionW = document.getElementById(
    "pbk_modalblockweb_title_optionW"
  );
  ...
  blocalumnLink.onclick = (event) => {
    ...
    if (prevtab === "blocalumne" || !alumne) {
      // Segon click o l'alumne no està escollit
      blockModalWeb.hide();
      const selectOptions = [];
  ...
  blocgrupLink.onclick = (event) => {
    ...
    if (prevtab === "blocgrup" || !grup) {
      // Segon click o el grup no està escollit
      blockModalWeb.hide();
      const selectOptions = [];
  ...
  hostSwitch.onchange = (event) => {
    if (event.target.checked) hostInput.removeAttribute("disabled");
    else hostInput.setAttribute("disabled", "disabled");

    menustate.host = event.target.checked;
  };

  pathnameSwitch.onchange = (event) => {
    if (event.target.checked) pathnameInput.removeAttribute("disabled");
    else pathnameInput.setAttribute("disabled", "disabled");

    menustate.pathname = event.target.checked;
  };

  searchSwitch.onchange = (event) => {
    if (event.target.checked) searchInput.removeAttribute("disabled");
    else searchInput.setAttribute("disabled", "disabled");
  ...
  normaButton.onclick = (event) => {
    ...
    if (
      hSelectDurada.value === "always" ||
      hSelectDurada.value === "today" ||
      hSelectDurada.value === "nopati"
    ) {
      text_confirmacio =
        "Segur que vols afegir una nova norma per " +
        (hSelectDurada.value === "today" ? "tot avui" : "sempre") +
        " a <i>" +
        normaWhoId +
        "</i>? Tingues en compte que això pot afectar a " +
        "altres professors o assignatures" +
        (hSelectDurada.value === "always" || hSelectDurada.value === "nopati"
          ? " ja que la norma que has " +
            "definit està <strong>sempre activa</strong>."
          : ".") +
        " És recomenable definir una durada més " +
        "concreta. Si saps el que fas, endavant!";
    }

    if (
      hostInput.value.includes("google") &&
      hostSwitch.checked &&
      !titleSwitch.checked
    ) {
      text_confirmacio =
        "Estàs segur que vols bloquejar un servei de Google a <i>" +
        normaWhoId +
        "</i>? " +
    ...
    if (
      !hostSwitch.checked &&
      (titleSwitch.checked || pathnameSwitch.checked || searchSwitch.checked)
    ) {
      text_confirmacio =
        "Estàs segur que vols bloquejar una pàgina a <i>" +
        normaWhoId +
        "</i> sense especificar-ne l'adreça amfitrió? " +
    ...
    if (hostInput.value === "newtab" && hostSwitch.checked) {
      text_confirmacio =
        "Estàs segur que vols bloquejar la pàgina de nova pestanya a <i>" +
        normaWhoId +
        "</i>? " +
...
export function obreDialogNormesWeb(whoid, who = "alumne") {
  ...
  for (const norma in normesWebInfo[whos][whoid]) {
    ...
    pencil.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
    ...
    eyeclose.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-slash-fill" viewBox="0 0 16 16">
    ...
    eyeopen.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
    ...
    pencil.onclick = (event) => {
      ...
      const webpage = {
        host: normesWebInfo[whos][whoid][norma].lines[0].host || "",
      ...
      obreDialogBloquejaWeb(
        ...
        "bloc" + who,
        {
          ...
          durada: reconstrueixDuradaOpcio(
            normesWebInfo[whos][whoid][norma].enabled_on
          ),
          editPrevious: {
            who: who,
            whoid: whoid,
    ...
    if (normesWebInfo[whos][whoid][norma].lines.length > 0) {
      for (const line of normesWebInfo[whos][whoid][norma].lines) {
        const div = document.createElement("div");
        div.setAttribute("class", "norma-line");
        if (line.host) div.innerHTML += "<b>Host:</b> " + line.host + " ";
        if (line.protocol)
          div.innerHTML += "<b>Protocol:</b> " + line.protocol + " ";
        if (line.search) div.innerHTML += "<b>Search:</b> " + line.search + " ";
        if (line.pathname)
          div.innerHTML += "<b>Pathname:</b> " + line.pathname + " ";
        if (line.title) div.innerHTML += "<b>Title:</b> " + line.title + " ";
        if (line.browser)
    ...
    if (web === "google.com") {
      ...
      for (const service of googleServices) {
        ...
        hservice.innerHTML = `<div class="llb-child-google-service">
                            <input type="checkbox" id="llb-weblist-input-${
                              service.title
                            }" data-url="${service.url}" ${
          service.default ? "checked" : ""
        }>
    ...
    button.onclick = (event) => {
      ...
      for (const commonPlace of commonPlaces) {
        if (commonPlace.url === web)
          document
            .getElementById("llb-add-" + commonPlace.title)
  ...
  confirma.onclick = (event) => {
    ...
    for (const web of llistaBlancaEnUs[grup]) {
      if (web.startsWith("[title]"))
        ...
        if (web === "google.com") {
          const googleServices = document.getElementsByClassName(
            "llb-child-google-service"
          );
          for (const service of googleServices) {
