...
function initializeAdminWebSocket(server) {
  ...
  io.on("connection", async (socket) => {
    ...
    try {
      ...
      socket.emit("alumnesMachine", machines);
      const normes = await normaController.getAllNormes2Web();
      socket.emit("normesWeb", normes);
    } catch (e) {
      logger.warn(`[ws-admin ${socket.id}] error paquet inicial: ${e.message}`);
    }
    ...
    socket.on("getHistorialWeb", async (msg) => {
      const historial = await historialController.getHistorialWeb(
        msg.alumne,
        msg.offset
      );
      socket.emit("historialWebAlumne", {
        alumne: msg.alumne,
        historial: historial,
      });
    });
    ...
    socket.on("getSearchHistorialWeb", async (msg) => {
      const historial = await historialController.searchHistorialWeb(
        msg.alumne,
        msg.search,
        msg.offset
      );
    ...
    socket.on("getEachBrowserLastUsage", async (msg) => {
      const lastUsage = await historialController.getEachBrowserLastUsage(
        msg.alumne
      );
      socket.emit("eachBrowserLastUsage", {
        alumne: msg.alumne,
        lastUsage: lastUsage,
      });
    });
    ...
    socket.on("getHistorialHostsSortedByUsage", async (msg) => {
      const sortedHistorial =
        await historialController.getHistorialHostsSortedByUsage(
          msg.alumne,
          msg.pastDays || 7
        );
      socket.emit("historialHostsSortedByUsage", {
        alumne: msg.alumne,
        sortedHistorial: sortedHistorial,
        days: msg.pastDays || 7,
      });
    });
    ...
    socket.on("updateNormaWeb", async (msg) => {
      await normaController.updateNorma2Web(
        msg.who,
        msg.whoid,
        msg.normaId,
        msg.severity,
    ...
    // Petició explícita (des de l'app d'escriptori) per obtenir de nou la llista de grups
    // Això és necessari perquè en el flux nou (multi-vista) el mòdul browsers
    // pot registrar els handlers DESPRÉS de la connexió del socket i es perd
    // l'emissió inicial de 'grupAlumnesList'.
    socket.on("getGrupAlumnesList", async () => {
      try {
        const grups = await alumneController.getGrupAlumnesList();
        socket.emit("grupAlumnesList", grups);
        logger.info(
        ...
        logger.warn("No s'ha pogut tornar a enviar grupAlumnesList", e);
      }
    });
    ...
    socket.on("getInitialData", async () => {
      try {
        ...
        logger.warn("No s'ha pogut re-emetre el paquet inicial", e);
      }
    });
